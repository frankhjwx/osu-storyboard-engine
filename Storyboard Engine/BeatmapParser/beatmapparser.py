    ...
    def parse_control_points(self):
        for obj in self.beatmap['hitObjects']:
            ...
            if obj['object_name'] == 'slider':
                ...
                timing = self.get_timing_point(obj["startTime"])
                if timing:
                    px_per_beat = float(self.beatmap["SliderMultiplier"]) * 100 * float(timing["velocity"])
                point_num = round(sliderlength / (px_per_beat / 8)) + 1
                beatlength = timing['beatLength']/8
                current_t = t0
                for x in range(obj['repeatCount']):
                    for i in range(point_num):
                        if x%2 == 0:
                            length = i * (px_per_beat / 8)
                        else:
                            length = (point_num - 1 - i) * (px_per_beat / 8)
